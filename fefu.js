; /* Start:"a:4:{s:4:"full";s:38:"/html/js/lodash.min.js?162080245973261";s:6:"source";s:22:"/html/js/lodash.min.js";s:3:"min";s:0:"";s:3:"map";s:0:"";}"*/
/**
 * @license
...
/* End */
;; /* Start:"a:4:{s:4:"full";s:80:"/local/components/dvfu/admission.spd/templates/.default/script.js?16239928938546";s:6:"source";s:65:"/local/components/dvfu/admission.spd/templates/.default/script.js";s:3:"min";s:0:"";s:3:"map";s:0:"";}"*/
var table;
const admissionMixin = {
    data() {
        return {
            loaded: false,
            admissionCampaignType: '',
            admissionCampaignTypeList: [],
            financingSourceList: [],
            studyFormList: [],
            implementationPlaceList: [],
    ...
    async mounted() {
        var self = this;
        let response = await BX.ajax.runComponentAction('dvfu:admission.spd',
            'getAdmissionCampaignTypeList', {
                mode: 'class',
            });
        this.admissionCampaignTypeList = response.data
        this.loaded = true;
    },

    watch: {
        async admissionCampaignType(newVal, oldVal) {
            this.clearValues(1);
            var response = await BX.ajax.runComponentAction('dvfu:admission.spd',
                'getFinancingSourceList', {
                    mode: 'class',
                    data: {
                        admissionCampaignType: newVal,
                    },
                });
            this.financingSourceList = response.data;
        },

        async financingSource(newVal, oldVal) {
            this.clearValues(2);
            var response = await BX.ajax.runComponentAction('dvfu:admission.spd',
                'getStudyFormList', {
                    mode: 'class',
                    data: {
                        admissionCampaignType: this.admissionCampaignType,
                        financingSource: newVal,
                    },
                });
            this.studyFormList = response.data
        },

        async studyForm(newVal, oldVal) {
            this.clearValues(3);
            var response = await BX.ajax.runComponentAction('dvfu:admission.spd',
                'getImplementationPlaceList', {
                    mode: 'class',
                    data: {
                        admissionCampaignType: this.admissionCampaignType,
            ...
            this.implementationPlaceList = response.data
        },

        async implementationPlace(newVal, oldVal) {
            this.clearValues(4);
            var response = await BX.ajax.runComponentAction('dvfu:admission.spd',
                'getTrainingDirectionList', {
                    mode: 'class',
                    data: {
                        admissionCampaignType: this.admissionCampaignType,
                        financingSource: this.financingSource,
                        studyForm: this.studyForm,
                        implementationPlace: newVal,
                    },
                });
            this.trainingDirectionList = response.data
        },
    },
    ...
    computed: {
        canShowStudents() {
            return (
                Boolean(this.admissionCampaignType) &&
                Boolean(this.financingSource) &&
                Boolean(this.studyForm) &&
                Boolean(this.implementationPlace) &&
                Boolean(this.trainingDirection)
            ) || false
        }
    },
    methods: {
        async getStudents() {
            if (this.canShowStudents) {
                var response = await BX.ajax.runComponentAction('dvfu:admission.spd',
                    'getStudents', {
                        ...
                        data: {
                            ...
                            financingSource: this.financingSource,
                            studyForm: this.studyForm,
                            implementationPlace: this.implementationPlace,
                            trainingDirection: this.trainingDirection,
                ...
                this.students = [];
                this.studentsGrouped = [];
            }
            return true;
        },
        getRowClass(type, item, cnt) {
            let trClass = '';
            ...
            if (Number.parseInt(item.enrolled) > 0) {
                trClass = ' blue';
            }
            return trClass;
        },
        clearValues(type) {
            ...
            let self = this;
            for (let i = type; i <= vars.length; i++) {
                self[vars[i]] = false;
            }
            self.searchString = '';
        },
        ...
        showStudents() {
            ...
            this.studentsGrouped = null;

            if (this.searchString != '') {
                this.studentsFiltered = this.students.filter(
                    el => (el.name.toLowerCase().indexOf(this.searchString.toLowerCase()) != -1)
                );
            } else {
                this.studentsFiltered = this.students;
            }

            if (this.financingSource == 'Полное возмещение затрат') {
                var items3 = this.studentsFiltered.filter(el => (el.category == 'На общих основаниях'));
                this.studentsGrouped = {
                    'type3': {
                        title: 'На общих основаниях',
                        vacantTitle: 'Договорных мест',
                        quota: items3[0] ? items3[0].dm : 0,
                        items: items3
                    },
                }
            } else {
                var items1 = this.studentsFiltered.filter(el => (el.category == 'Имеющие особое право'));
                var items2 = this.studentsFiltered.filter(el => (el.category == 'Целевой прием'));

                var items3 = this.studentsFiltered.filter(el => (
                    el.category == 'Без вступительных испытаний'
                ));
                items3 = items3.concat(
                    this.studentsFiltered.filter(el => (
                        el.category == 'На общих основаниях'
                    ))
                );
                ...
                this.studentsGrouped = {
                    'type1': {
                        title: 'Особая квота',
                        vacantTitle: 'Особая квота',
                        quota: items1[0] ? items1[0].oq : 0,
                        items: items1
                    },
                    'type2': {
                        title: 'Целевой прием',
                        vacantTitle: 'Целевая квота',
                        quota: items2[0] ? items2[0].cq : 0,
                        items: items2
                    },
                    'type3': {
                        title: 'На общих основаниях',
                        vacantTitle: 'Бюджетных мест (конкурс)',
                        quota: items3[0] ? items3[0].bm : 0,
                        items: items3
                    },
                }
            }
...
/* End */
;; /* /html/js/lodash.min.js?162080245973261*/ ; /* /local/components/dvfu/admission.spd/templates/.default/script.js?16239928938546*/